...
from yolov6.utils.checkpoint import load_state_dict, save_checkpoint, strip_optimizer,strip_optimizer_middle
...
class Trainer:
    ...
    # Training Process
    def train(self):
        time_meter = TimeMeter(self.max_epoch-self.start_epoch)
        try:
    ...
    def after_epoch(self):
        if self.main_process:
            LOGGER.info(('%10s' + ' %10.4g' + '%10.4g' * self.loss_num) % (f'{self.epoch}/{self.max_epoch - 1}', \
        ...
        if self.main_process:
            ...
            if is_val_epoch:
                ...
                if (self.ap>=self.best_ap):
                    ...
                    self.best_05map=self.evaluate_results[0]
                    self.best_95map = self.evaluate_results[1]
                    LOGGER.info(f"Best_Epoch/Epochs: {self.best_epoch}/{self.max_epoch} | Best_mAP@0.5: {self.evaluate_results[0]} | Best_mAP@0.50:0.95: {self.evaluate_results[1]}")
                else:
                    LOGGER.info(f"Best_Epoch/Epochs: {self.best_epoch}/{self.max_epoch} | Best_mAP@0.5: {self.best_05map} | Best_mAP@0.50:0.95: {self.best_95map}")
                #####
    ...
    def before_train_loop(self):
        ...
        self.best_05map,self.best_95map=0.0,0.0
        self.best_epoch=0
    ...
    def strip_middle_model(self):
        if self.main_process:
            save_ckpt_dir = osp.join(self.save_dir, 'weights')
            save_middleckpt_dir = osp.join(self.save_dir, 'weights', 'middle')
            if not os.path.exists(save_middleckpt_dir):
                os.makedirs(save_middleckpt_dir)
            strip_optimizer_middle(save_ckpt_dir,save_middleckpt_dir ,self.epoch)  # strip optimizers for saved pt model
