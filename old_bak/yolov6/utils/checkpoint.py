...
def strip_optimizer_middle(ckpt_dir, middleckpt_dir, epoch):
    """Delete optimizer from saved checkpoint file"""
    for s in ['best']:
        ckpt_path = osp.join(ckpt_dir, '{}_ckpt.pt'.format(s))
        middle_ckpt_path = osp.join(middleckpt_dir, '{}_middle.pt'.format(s))
        if not osp.exists(ckpt_path):
            continue
        ckpt = torch.load(ckpt_path, map_location=torch.device('cpu'))
        ckptdeepcopy=deepcopy(ckpt)
        if ckptdeepcopy.get('ema'):
            ckptdeepcopy['model'] = ckptdeepcopy['ema']  # replace model with ema
        for k in ['optimizer', 'ema', 'updates']:  # keys
            ckptdeepcopy[k] = None
        ckptdeepcopy['epoch'] = epoch
        ckptdeepcopy['model'].half()  # to FP16
        for p in ckptdeepcopy['model'].parameters():
            p.requires_grad = False
        torch.save(ckptdeepcopy, middle_ckpt_path)
